...
function CreateSwimmer({ params }: Readonly<{ params: ParamsInterface }>) {
  ...
  const hasHandledUser = useRef(false); // Ref to track if handleUser has been called
    ...
    async (userData: any) => {
      ...
      if (userData) {
        ...
        try {
          const response = await axios.post("/api/create-swimmer", {
            userData,
          });
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 py-10 px-2">
      <div
        className={`relative w-full max-w-2xl mx-auto bg-white rounded-2xl shadow-2xl p-8 transition-opacity duration-300 ${
      ...
      >
        ...
        {!loading && userData?(
          <div className="overflow-hidden rounded-xl border border-gray-200 shadow p-5 bg-gray-50">
            <table className="min-w-full border-separate border-spacing-y-2 border-spacing-x-2">
              <thead>
                <tr>
                  <th className="py-3 text-sm font-bold text-gray-900 text-left">
                    Field
                  </th>
                  <th className="py-3 text-sm font-bold text-gray-900 text-left">
                    Value
              ...
              <tbody>
                <tr>
                  ...
                  <td className="py-3 text-sm text-gray-700">
                    {userData?.swimmerFirstName ?? "First Name"}{" "}
                    {userData?.swimmerLastName ?? "Last Name"}
                  </td>
                </tr>
                <tr>
                  <td className="py-3 text-sm font-bold text-gray-900">
        ...
        {!loading && (
          <div className="text-center mt-8">
            ...
            <a
              href="/"
              className="inline-block px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-lg shadow transition-colors duration-200"
            >
