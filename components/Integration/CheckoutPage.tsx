...
const CheckoutForm = (props: any) => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // if(userData){
            //     const swimmerResponse = await axios.post('/api/create-swimmer', { userData });
            // }
        } catch (err: any) {
    ...
    <>
        <form onSubmit={handleSubmit} className={`w-full max-w-lg mx-auto p-4 bg-white rounded-lg mt-32  shadow-2xl ${loading?"opacity-15":"" }`}>
                ...
                <div className="  rounded-lg ">
                    ...
                    <div>
                        ...
                        <div className="flex justify-between px-3 pt-4 pb-3">
                            <div>
                                <span aria-disabled={loading} className="text-gray-700 text-m p-2 rounded font-normal leading-9 hover:bg-red-300 hover:cursor-pointer " onClick={() => router.back()}>Go back</span>
                            </div>
            ...
            <p className="mt-6 ml-12 text-center text-gray-500 dark:text-gray-400 sm:mt-8 lg:text-left">
        Payment processed by <a href="#" title="" className="font-medium text-primary-700 underline hover:no-underline dark:text-primary-500">Instamojo Payment Gateway</a>
      </p>
        </form>
